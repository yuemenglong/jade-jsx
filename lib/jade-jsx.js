...
function getJadeCode(node, ast) {
    switch (node.type) {
        ...
        case "Identifier":
            return getIdentifierValueAndRemove(ast, node.name)
        default:
...
function jadeToJsx(src, fn) {
    ...
    list.map(function(node) {
        ...
        var jade = getJadeCode(jadeNode, root);
        var fn = compile(jade);
...
if (require.main == module) {
    var src = `var asd='div';jade(asd)`;
    // var ast = getAst(src);
    // getTargetNodesWithRelation(ast);
    // getIdentifierValueAndRemove(ast, "asd");
    // var jsx = getSrc(ast);
    // console.log(jsx);
